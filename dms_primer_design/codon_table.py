...
def codons_list(seq):
    """Input string and return list of codons"""
...
def codon_substitutions(wt_codon, position, iupac_codon):
    """Return string of codon substitutions at a given position"""
    nuc_lists = [list(rev_iupac_dict[n]) for n in iupac_codon]
    codon_list = [''.join(i) for i in list(itertools.product(*nuc_lists))]
    variants_list = [f'{wt_codon}{position}{codon}' for codon in codon_list]
